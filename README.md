...
- A six-lane procedural arrangement system—DRUMS, BASS, SYNTH, HARMONY, LEAD and TEXTURE—runs above the star material. Its director moves through INTRO → GROOVE → BUILD → OPEN → MOTIF → BREAK → RETURN → TAIL, replacing generated pattern strategies rather than stacking permanent loops.
- PLAY now has two keyboard meanings. **STAR** preserves direct Press/Hold/Release performance. **ARRANGE** turns 3–8 deterministically extracted bright anchors, branch points and representative stars into quantized PatternVariant selectors for one TrackLane. A manual lane remains overridden for eight bars while the other five lanes continue under the automatic director; `AUTO` returns every lane immediately.
- A bounded Canvas 2D `ParticleField` reads the same `VisualMusicState` as the audio engine. Kick events produce restrained radial impulses, percussion creates micro-sparkles, bass drives slow field breathing, synth/lead create directional trails, harmony extends lifetime/spread, and texture controls granular density. Chinese, Western and Indian profiles use radial, linear and orbital motion respectively without changing colour.
