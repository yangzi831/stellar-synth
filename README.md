...
- A six-lane procedural arrangement system—DRUMS, BASS, SYNTH, HARMONY, LEAD and TEXTURE—runs above the star material. The rhythm floor is fixed to a clear 16-step 4/4 bar at 150 BPM. Cultural asymmetry remains available to percussion, motifs and fragment phrases without displacing the four-on-the-floor body pulse.
...
- The master chain now uses a raised input stage, gentle compression, makeup gain and a final limiter set near -1 dBFS. A lightweight analyser exposes test-only running RMS and peak diagnostics without driving musical or visual behaviour.
...
- 150 BPM Web Audio engine derived from D5, layering the original plucks with 8-bar ambient pads, stable bass/synth/melodic phrases, four-on-the-floor drums, pumping dynamics and occasional glitch. Six synth recipes—analog pluck, saw sequence, acid-like resonant sequence, soft poly, dark pulse and FM/metallic—are generated with Web Audio rather than copied from hardware. Percussion and glitch use procedurally generated buffers; no external sample pack or recording is bundled.
...
- Guided LOOP is deliberately a performance looper rather than a DAW: it records quantized trigger choices, supports restarting and clearing the current layer, but does not yet provide per-note editing, undo history, saving/export or MIDI clock.
